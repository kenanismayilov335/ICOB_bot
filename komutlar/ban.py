    ...
    if message.chat.type != "private":
        if message.reply_to_message:
            ...
            if client.get_chat_member(message.chat.id, message.from_user.id)["status"] in yetkiler:
                ...
                else:message.reply(f"""Banlamak istediğiniz kişinin zaten **__{datetime.utcfromtimestamp(client.get_chat_member(message.chat.id, message.reply_to_message.from_user.id)["until_date"])}__** süresine kadar banı var.""")
